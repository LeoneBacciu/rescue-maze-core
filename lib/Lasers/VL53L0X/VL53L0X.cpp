...
// mode.
bool VL53L0X::init(TwoWire *wire, bool io_2v8) {
    bus = wire;
    ...
    // The SPAD map (RefGoodSpadMap) is ReadF by VL53L0X_get_info_from_device() in
    // the API, but the same data seems to be more easily readable from
    // GLOBAL_CONFIG_SPAD_ENABLES_REF_0 through _6, so ReadF it from there
...
bool VL53L0X::setVcselPulsePeriod(vcselPeriodType type, uint8_t period_pclks) {
    ...
    // the corresponding timeout must be ReadF from the device using
...
// Did a timeout occur in one of the ReadF functions since the last call to
// timeoutOccurred()?
...
void VL53L0X::setHighPrecision() {
    ...
    setMeasurementTimingBudget(60000);
}
